...
export const sketchTools = [
  ...
  {
    name: 'create_sketch',
    description: 'REQUIRES WINDOWS + SOLIDWORKS. Open a new sketch on a plane (Front, Top, Right) or a planar face. To sketch on an arbitrary face, provide facePoint — a point in mm on the target planar face. The sketch stays in edit mode — add geometry with sketch_line, sketch_circle, etc., then call exit_sketch when done. NOTE: When using facePoint, the sketch 2D coordinate system depends on face orientation — use get_sketch_geometry after placing first entity to verify coordinates if results look wrong.',
    inputSchema: z.object({
      ...
      facePoint: z.object({
        x: z.number().describe('X coordinate in mm on the target face'),
        y: z.number().describe('Y coordinate in mm on the target face'),
        z: z.number().describe('Z coordinate in mm on the target face')
      }).optional().describe('Point on a planar face to create the sketch on (overrides plane selection). Coordinates in mm.'),
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        // Face selection: use VBA macro (winax COM fails for face SelectByID2)
        if (args.facePoint) {
          const { x, y, z } = args.facePoint;
          const xM = (x / 1000).toFixed(10);
          const yM = (y / 1000).toFixed(10);
          const zM = (z / 1000).toFixed(10);
...
Sub main()
    ...
    swModel.Extension.SelectByID2 "", "FACE", ${xM}, ${yM}, ${zM}, False, 0, Nothing, 0
    Dim selCount As Long
    selCount = swModel.SelectionManager.GetSelectedObjectCount2(-1)

    If selCount < 1 Then
        WriteLog "{""error"": ""No face found at (${x}, ${y}, ${z})mm""}"
        Exit Sub
    End If

    swModel.InsertSketch2 True

    Dim skName As String: skName = ""
    Dim sk As Object
    Set sk = swModel.SketchManager.ActiveSketch
    If Not sk Is Nothing Then skName = sk.Name

    WriteLog "{""success"": true, ""sketchName"": """ & skName & """}"
End Sub
...
`;
          ...
          if (!result.success) {
            throw new Error(`Face sketch macro failed: ${result.error || 'unknown'}`);
          }
          let parsed: any;
          ...
          const sketchName = parsed?.sketchName || 'Sketch';

          return {
            success: true,
            sketchName,
            plane: 'Face',
            facePoint: args.facePoint,
            message: `Sketch created on face at (${x}, ${y}, ${z})mm`
          };
        }
  ...
  {
    name: 'sketch_line',
    description: 'REQUIRES WINDOWS + SOLIDWORKS. Add a line segment between two points in the active sketch. Coordinates are in mm. For extrusion, lines must form a closed profile (start of last line = end of first line). A sketch must be open (create_sketch or edit_sketch first).',
    inputSchema: z.object({
      start: z.object({
